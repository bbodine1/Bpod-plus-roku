sub init()
  ...
  m.top.observeField("active", "onActiveChanged")
end sub
...
sub startPreview()
  m.settings = m.top.settings
  if m.settings = invalid then m.settings = DefaultSettings()
...
sub refreshWeather()
  ...
  m.weatherTask.units = m.settings.units
